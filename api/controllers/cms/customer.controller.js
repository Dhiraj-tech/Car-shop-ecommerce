...
class customerController {
    ...
    store = async(req,res,next)=>{
        try{
            const {name,email,password,confirm_password,address,status} = req.body

            if (password == confirm_password){
                const hash = await genPassword(password)

            await User.create({name,email,address,status,type:'Customer',password:hash})
    ...
    update = async(req,res,next)=>{
        try{
            ...
            await User.findByIdAndUpdate(req.params.id,{name,email,address,status})
            res.json({
